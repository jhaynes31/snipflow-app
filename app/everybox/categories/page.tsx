...
import { CategoryForm } from "@/components/everybox/CategoryForm";
import { partnerName, useEveryBox } from "@/components/everybox/context";
import { Btn, PageTitle, Spinner, useAction } from "@/components/everybox/ui";
            ...
            {editing === c._id ? (
              ...
              <div className="flex flex-wrap items-center gap-3">
                ...
                <div className="min-w-0 flex-1">
                  <Link href={`/everybox/category/${c._id}`} className="font-semibold">
                    {c.name}
                  </Link>
                  <div className="text-xs eb-muted">
                    {describeCadence(c.idealCadenceDays)} · tended by {partnerName(partners, c.tenderId, partner._id)}
                  </div>
                </div>
                <div className="flex items-center gap-1">
                  <Btn small variant="ghost" aria-label="Move up" disabled={busy || i === 0} onClick={() => move(categories, i, -1)}>
                    ↑
                  </Btn>
                  <Btn small variant="ghost" aria-label="Move down" disabled={busy || i === categories.length - 1} onClick={() => move(categories, i, 1)}>
                    ↓
                  </Btn>
