...
# Load runtime secrets (DATABASE_URL, ANTHROPIC_API_KEY, ADMIN_PASSWORD, etc.) from .env so the
# deployed serverless functions receive them at build/runtime time, matching how
# DATABASE_URL is passed below. .env is gitignored, so secrets stay out of the repo.
...
# Resolve the project id from .vercel/project.json (the --name flag is deprecated).
PROJECT_ID="$(bun -e 'const p=require("./.vercel/project.json");process.stdout.write(p.projectId||"")' 2>/dev/null || true)"
if [ -z "$PROJECT_ID" ]; then
  echo "warning: could not resolve project id from .vercel/project.json" >&2
  exit 0
fi
curl -sf -X PATCH "https://api.vercel.com/v9/projects/${PROJECT_ID}${TEAM_QS}" \
