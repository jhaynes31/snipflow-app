import { sql } from "~/db";
import { isOutcome, parsePersona, text, type Practitioner, type SectionInput, type StartInput } from "~/lib/practiceInput";
...
export const MODEL = () => process.env.PRACTICE_MODEL || PRACTICE_MODEL_DEFAULT;
...
export function ensurePracticeTables(): Promise<void> {
  if (!ready) {
    ready = (async () => {
      ...
      // Phase 4: a recruit chooses whether John may read their transcript (Section 9).
      await sql()`ALTER TABLE practice_sessions ADD COLUMN IF NOT EXISTS shared_with_john BOOLEAN NOT NULL DEFAULT FALSE`;
      await sql()`
...
export interface PracticeSession {
  ...
  /** The recruit who practiced, when practitioner is "recruit". Never sent to the AI. */
  recruitId: number | null;
  /** John marked this one of his own sessions as an example recruits may read. */
  sharedAsExample: boolean;
  /** A recruit let John read this transcript. */
  sharedWithJohn: boolean;
  /** Set on John's side when a recruit's transcript is kept private: counts and outcome only. */
  hidden?: boolean;
  turns: number;
  maxTurns: number;
  endedAt: string | null;
...
export function rowToSession(r: Record<string, unknown>): PracticeSession {
  ...
  return {
    ...
    recruitId: r.recruit_id == null ? null : Number(r.recruit_id),
    sharedAsExample: r.shared_as_example === true || r.shared_as_example === "t",
    sharedWithJohn: r.shared_with_john === true || r.shared_with_john === "t",
...
/** What a list needs: no transcript, no debrief. Safe to show John for a recruit's private session. */
...
export function summarize(s: PracticeSession): SessionSummary {
  return { id: s.id, mode: s.mode, conversation: s.conversation, personaName: s.persona.name, temperament: s.temperament, difficulty: s.difficulty, outcome: s.outcome, turns: s.turns, practitioner: s.practitioner, recruitId: s.recruitId, sharedAsExample: s.sharedAsExample, sharedWithJohn: s.sharedWithJohn, endedAt: s.endedAt, createdAt: s.createdAt };
}

/** John's transcript, or a recruit's once they chose to share it. Otherwise the shell of the session. */
export function forJohn(s: PracticeSession): PracticeSession {
  if (s.practitioner !== "recruit" || s.sharedWithJohn) return s;
  return { ...s, transcript: [], debrief: null, progress: [], pendingInterrupt: false, hidden: true };
}

export const speakerFor = (s: PracticeSession): Speaker => (s.practitioner === "recruit" ? TRAINEE : JOHN);
...
export async function loadProfiles(): Promise<ClientProfile[]> {
...
/** Section 4: a fictional composite from the profile description only. Shared by John's and a recruit's setup. */
export async function inventPersona(data: { profileId: number; conversation: Conversation; avoidNames: string[] }): Promise<{ ok: boolean; error?: string; persona?: Persona; snapshot?: string }> {
...
async function systemFor(s: PracticeSession): Promise<string> {
  const sp = speakerFor(s);
  let base = playSystemPrompt({ persona: s.persona, conversation: s.conversation, temperament: s.temperament, difficulty: s.difficulty, maxTurns: MAX_TURNS }, sp);
  if (s.presentation) base += presentationBlock(s.presentation.name, s.presentation.sections, sp);
  return s.conversation === "recruiting" ? base + recruitingTrustBlock(await trustFacts(), sp) : base;
}

function chatMessages(transcript: TranscriptEntry[], opening: string, sp: Speaker): Array<{ role: "user" | "assistant"; content: string }> {
  const msgs: Array<{ role: "user" | "assistant"; content: string }> = [{ role: "user", content: opening }];
  for (const m of transcript) {
    if (m.role === "john") msgs.push({ role: "user", content: m.kind === "said" ? `[During the section, ${sp.name} said:] ${m.text}` : m.text });
...
async function personaTurn(s: PracticeSession, cue?: string): Promise<PersonaReply | null> {
  const sp = speakerFor(s);
  const opening = s.presentation ? presentationOpeningCue(s.presentation.sections[0], sp) : openingCue(sp);
  const msgs = chatMessages(s.transcript, opening, sp);
...
export async function loadSession(id: number): Promise<PracticeSession | null> {
...
export async function storeTranscript(s: PracticeSession, extra?: { outcome?: Outcome; ended?: boolean }): Promise<void> {
  if (extra?.ended) {
...
export const AS_JOHN: Practitioner = { practitioner: "john", recruitId: null };

/** Section 7.1: creates the session and asks the persona to open. Shared by John and recruits. */
export async function createSession(data: StartInput, who: Practitioner): Promise<SessionResult> {
  ...
  try {
    ...
    const rows = (await sql()`
      INSERT INTO practice_sessions (mode, conversation, profile_id, profile_snapshot, persona, temperament, difficulty, transcript, practitioner, recruit_id, script_id, script_version, script_snapshot, sections_covered)
      VALUES (${data.mode}, ${data.conversation}, ${data.profileId}, ${data.profileSnapshot}, ${JSON.stringify(data.persona)}, ${data.temperament}, ${data.difficulty}, '[]', ${who.practitioner}, ${who.recruitId}, ${scriptId}, ${scriptVersion}, ${snapshot}, '[]')
...
export async function sayTurn(s: PracticeSession, line: string): Promise<SessionResult> {
  ...
  s.transcript.push({ role: "john", text: line, at: new Date().toISOString(), ...(openSection ? { sectionId: openSection.sectionId } : {}) });
...
export async function giveHint(s: PracticeSession): Promise<SessionResult> {
  const sp = speakerFor(s);
  const reply = await callClaudeChat({ system: hintSystemPrompt(sp), messages: [{ role: "user", content: hintUserPrompt(s.persona, s.transcript, sp) }], maxTokens: 200, model: MODEL(), tag: "practice-hint" });
...
export async function finishSession(s: PracticeSession, outcome: Outcome): Promise<SessionResult> {
  if (!s.endedAt) {
    s.transcript.push({ role: "system", text: s.practitioner === "recruit" ? "The practice was ended." : "John ended the practice.", at: new Date().toISOString(), kind: "note" });
    await storeTranscript(s, { ended: true, outcome });
  }
  return { ok: true, session: { ...s, outcome: s.endedAt ? s.outcome : outcome, endedAt: s.endedAt ?? new Date().toISOString() } };
}

export interface PracticeOverview {
  sessions: SessionSummary[];
  recruits: Array<{ recruitId: number; sessions: number; outcomes: Partial<Record<Outcome, number>>; lastAt: string; shared: SessionSummary[] }>;
}
...
export async function loadRubrics(): Promise<Rubrics> {
  await ensurePracticeTables();
...
export async function buildDebrief(s: PracticeSession, refresh = false): Promise<{ ok: boolean; error?: string; debrief?: Debrief }> {
  if (!s.endedAt) return { ok: false, error: "The session has not ended yet." };
  if (s.debrief && !refresh) return { ok: true, debrief: s.debrief };
  const sp = speakerFor(s);
  ...
  const reply = await callClaudeChat({ system: debriefSystemPrompt(sp), messages: [{ role: "user", content: debriefUserPrompt({ conversation: s.conversation, persona: s.persona, outcome, rubric, transcript: s.transcript, presentationLines: presentation?.lines }, sp) }], maxTokens: 900, model: MODEL(), tag: "practice-debrief" });
...
export function rowToPresentation(r: Record<string, unknown>): Presentation {
...
export async function applySectionEvent(s: PracticeSession, data: SectionInput): Promise<SessionResult> {
  const sp = speakerFor(s);
  ...
  try {
    ...
    if (data.event === "midpoint") {
      ...
      const reply = await personaTurn(s, target ? interruptionCue(section, target, sp) : disengageCue(section, sp));
    ...
    if (data.event === "delivered") {
      ...
      const reply = await personaTurn(s, sectionReactionCue(section, data.said, isLast, sp));
      if (reply) {
