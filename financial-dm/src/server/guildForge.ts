...
- Never use the protected titles ${GUILD_CONFIG.titleFlagWords.join(", ")}. John may be described as a financial advisor, but never offer the role to recruits as ${GUILD_CONFIG.roleTitleFlagWords.join(", ")}: a new recruit starts as "${facts.roleTitle}". Investment work is only ever "an optional path you can get licensed for later"; never imply anyone can advise on or sell investments before they are properly licensed; never mention returns, performance, or products.
...
- John's title is exactly "Licensed Term Life Agent". He works with ${GUILD_CONFIG.presentedBy}.
