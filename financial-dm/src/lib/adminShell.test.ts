...
describe("admin shell", () => {
  test("tabs are in the fixed order and every built one has a route", () => {
    expect(ADMIN_TABS.map((t) => t.id)).toEqual(["home", "leads", "quests", "forge", "studio", "practice", "quizzes", "scripts", "guild", "howto", "settings"]);
  ...
  test("breadcrumbs name the deep screen and stay empty on a main view", () => {
    ...
    expect(crumb("/admin/practice/12", {})?.sub).toEqual(["Practice session"]);
    expect(activeTab("/admin/practice/12")?.id).toBe("practice");
  });
