...
import { Btn, ErrorNote, Field, PageTitle, Spinner, useAction } from "@/modules/every-box/components/ui";
...
export function Commitments() {
  ...
  const [assignedToIds, setAssignedToIds] = useState<Id<"ebPartners">[]>([partner._id]);
  function toggleAssignee(id: Id<"ebPartners">) {
    setAssignedToIds((ids) => (ids.includes(id) ? ids.filter((x) => x !== id) : [...ids, id]));
  }
  ...
  const forMe = data.open.filter(
    (c) => c.status === "proposed" && isAssignee(c, partner._id) && c.proposedBy !== partner._id,
  );
  const forOther = data.open.filter((c) => c.status === "proposed" && c.proposedBy === partner._id);
    ...
    <div className="eb-container max-w-2xl">
      ...
      {open && (
        ...
        >
          ...
          <Field
            label="Who's it for?"
            hint={other ? "Tap one or both. Either of you can check in on a shared one." : "Once your partner joins, you'll be able to propose things for them too."}
          >
            <div className="flex flex-wrap gap-2">
              {partners.map((p) => (
                <button
                  ...
                  type="button"
                  className="eb-chip"
                  style={assignedToIds.includes(p._id) ? { outline: "2px solid var(--eb-accent)", color: "var(--eb-accent)" } : undefined}
                  onClick={() => toggleAssignee(p._id)}
                  aria-pressed={assignedToIds.includes(p._id)}
                >
          ...
          <div className="flex gap-2">
            ...
            <Btn type="submit" disabled={busy || !title.trim() || assignedToIds.length === 0}>
              {assignedToIds.every((id) => id === partner._id)
                ? "Commit"
                : `Propose to ${other?.displayName ?? "your partner"}`}
            </Btn>
          </div>
        </form>
      )}
