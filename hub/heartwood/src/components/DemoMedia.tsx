...
export function DemoMedia({ exercise, slow = false, height = 220 }: { exercise: Exercise; slow?: boolean; height?: number }) {
  ...
  if (media.type === 'video' || (media.type === 'custom' && blobUrl)) {
    return (
      <div className="relative">
        <video ref={videoRef} src={media.type === 'custom' ? blobUrl! : assetUrl(media.src)} style={style} loop muted playsInline autoPlay aria-label={`${exercise.name} demonstration`} />
  ...
  if (media.type === 'gif') {
    return (
      <figure className="m-0">
        <img src={assetUrl(media.src)} alt={`${exercise.name} demonstration`} style={style} loading="lazy" />
  ...
  return (
    <div style={{ ...style, display: 'grid', placeItems: 'center', overflow: 'hidden' }}>
      <img src={assetUrl(media.src)} alt={`${exercise.name}: ${exercise.stance.replace('-', ' ')} position`} style={{ height: '100%', objectFit: 'contain', animationPlayState: paused ? 'paused' : 'running' }} />
    </div>
  );
}
