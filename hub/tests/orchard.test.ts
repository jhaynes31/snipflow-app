...
describe("my signals", () => {
  it("holds Jen's list in full, each with a tell, a test, a response and a twin", () => {
    ...
    for (const k of ["neverInitiates", "gossip", "confidence", "wontBeHelped", "asksDoesntShare", "bypassing", "godScapegoat", "oneTopic", "victim", "constantMotion", "avoidant"]) assert.ok(SIGNALS.some((s) => s.key === k), k);
