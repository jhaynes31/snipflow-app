...
import { personSuffix } from '@/app/person'

// Seed markers are per person, like the database itself.
const SEED_KEY = `lr:seeded:v1${personSuffix()}`
const RINGS_KEY = `lr:rings-seeded:v1${personSuffix()}`
const GRACE_KEY = `lr:grace-seeded:v1${personSuffix()}`
const TRUTHS_V2_KEY = `lr:truths-seeded:v2${personSuffix()}`
